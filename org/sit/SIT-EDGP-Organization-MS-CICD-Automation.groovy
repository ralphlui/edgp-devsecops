pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_ORG_MANAGEMENT')
        S3_BUCKET = "edgp-app-sit/org"
        BUILD_DIR = "/var/lib/jenkins"
        PROJECT_KEY = "ralphlui_edgp-org-management"
        IMAGE_NAME = "edgp-app-org"
    ...
    stages {
        stage('Run unit tests') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: '46331406-ad64-4cf5-a045-9f5de2ad6cfc', url: 'git@github.com:ralphlui/edgp-org-management.git']])
                sh 'mvn -B clean test'
            }
        }
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    def filename = 'docker_version_org.txt'
                    ...
                    if (fileExists(concatenatedPath)) {
                        ...
                        sh "cp ./docker_version_org.txt ${VERSION_FILE_PATH}"
                    }
