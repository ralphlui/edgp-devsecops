pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_WEB')
        S3_BUCKET = "edgp-app-sit/web"
        BUILD_DIR = "/var/lib/jenkins"
        PROJECT_KEY = "ralphlui-edgp-web"
        ORGANIZATION_KEY = "ralphlui-enterprise-data-governance-platform"
        IMAGE_NAME = "edgp-app-web"
    ...
    stages {
        ...
        stage('Checkout & Install dependencies') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: '46331406-ad64-4cf5-a045-9f5de2ad6cfc', url: 'git@github.com:ralphlui/edgp-web.git']])
            ...
            steps {
                script {
                    ...
                    dir("${BUILD_DIR}/workspace/${pipelineName}") {
                        sh """
                            ${SCA_SCAN} --project 'Enterprise Data Governance Platform  (${IMAGE_NAME})' --scan . --nvdApiKey ${API_KEY} --disableAssembly --disableYarnAudit
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    cp /var/lib/jenkins/nginx/default.d ./
