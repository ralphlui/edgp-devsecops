pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_NOTIFICATION')
        S3_BUCKET = "edgp-app-sit/notification"
        BUILD_DIR = "/var/lib/jenkins"
        PROJECT_KEY = "ralphlui_edgp-notification"
        IMAGE_NAME = "edgp-app-notification"
    ...
    stages {
        stage('Run unit tests') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: '46331406-ad64-4cf5-a045-9f5de2ad6cfc', url: 'git@github.com:ralphlui/edgp-notification.git']])
                sh 'mvn -B clean test'
            }
        }
            ...
            steps {
                script {
                    ...
                    dir("${BUILD_DIR}/workspace/${pipelineName}/target") {
                        sh """
                            ${SCA_SCAN} --project 'Enterprise Data Governance Platform (${IMAGE_NAME})' --scan . --nvdApiKey ${API_KEY} --noupdate
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    def filename = 'docker_version_notification.txt'
                    ...
                    if (fileExists(concatenatedPath)) {
                        ...
                        sh "cp ./docker_version_notification.txt ${VERSION_FILE_PATH}"
                    }
        ...
        stage('Trigger DAST Pipeline') {
            ...
            steps {
                build job: 'SIT-EDGP-DAST-CD-Automation', wait: false, parameters: [
                    string(name: 'APP', value: 'edgp-app-admin')
                ]
            }
        }
    }
