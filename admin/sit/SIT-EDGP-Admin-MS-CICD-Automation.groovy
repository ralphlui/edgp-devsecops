pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_ADMIN_MANAGEMENT')
        S3_BUCKET = "edgp-app-sit/admin"
        BUILD_DIR = "/var/lib/jenkins"
        PROJECT_KEY = "ralphlui_edgp-admin-management"
        IMAGE_NAME = "edgp-app-admin"
    ...
    stages {
        stage('Run unit tests') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: '46331406-ad64-4cf5-a045-9f5de2ad6cfc', url: 'git@github.com:ralphlui/edgp-admin-management.git']])
                sh 'mvn -B clean test'
            }
        }
            ...
            steps {
                script {
                    ...
                    dir("${BUILD_DIR}/workspace/${pipelineName}/target") {
                        sh """
                            ${SCA_SCAN} --project 'Enterprise Data Governance Platform (${IMAGE_NAME})' --scan . --nvdApiKey ${API_KEY} --disableYarnAudit
        ...
        stage ('Build Docker image') {
            steps {
                script {
                    ...
                    def filename = 'docker_version_admin.txt'
                    ...
                    if (fileExists(concatenatedPath)) {
                        ...
                        sh "cp ./docker_version_admin.txt ${VERSION_FILE_PATH}"
                    }
