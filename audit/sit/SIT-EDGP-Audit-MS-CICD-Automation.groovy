pipeline {
    ...
    environment {
        SONAR_TOKEN = credentials('SONAR_TOKEN_AUDIT_LOGGING')
        S3_BUCKET = "edgp-app-sit/audit"
        BUILD_DIR = "/var/lib/jenkins"
        PROJECT_KEY = "ralphlui_edgp-audit-logging"
        IMAGE_NAME = "edgp-app-audit"
    ...
    stages {
        stage('Run unit tests') {
            steps {
                checkout scmGit(branches: [[name: '*/development']], extensions: [], userRemoteConfigs: [[credentialsId: '46331406-ad64-4cf5-a045-9f5de2ad6cfc', url: 'git@github.com:ralphlui/edgp-audit-logging.git']])
                sh 'mvn -B clean test'
            }
        }
